...
export function mapToSheet1Row(data: Data): unknown[] {
  return [
    ...
    /* N  Aged over 75               */ data.ageOver75 || boolToYesNo(data.isOver75),
    /* O  Conservation/Listed        */ '',
    /* P  Roof Works Planned         */ data.roofWorks || boolToYesNo(data.roofWorksPlanned),
    /* Q  Income >£15K               */ data.income || boolToYesNo(data.incomeOver15k),
    /* R  Appointment Within 5 Days  */ calcAppointmentWithin5Days(data),
    /* S  Likely to pass credit check*/ data.creditCheck || boolToYesNo(data.likelyToPassCreditCheck),
