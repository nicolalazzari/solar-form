...
export function InactivityProvider({ children }) {
  ...
  const buildPayload = useCallback((data, overrides = {}) => {
    ...
    return {
      ...
      ageOver75: data.isOver75 === true ? 'Yes' : data.isOver75 === false ? 'No' : '',
      roofWorksPlanned: data.roofWorksPlanned,
      roofWorks: data.roofWorksPlanned === true ? 'Yes' : data.roofWorksPlanned === false ? 'No' : '',
      incomeOver15k: data.incomeOver15k,
      income: data.incomeOver15k === true ? 'Yes' : data.incomeOver15k === false ? 'No' : '',
      likelyToPassCreditCheck: data.likelyToPassCreditCheck,
      creditCheck: data.likelyToPassCreditCheck === true ? 'Yes' : data.likelyToPassCreditCheck === false ? 'No' : '',
      bookingId: data.selectedSlot?.startTime || '',
      bookingReference: data.selectedSlot?.startTime || '',
